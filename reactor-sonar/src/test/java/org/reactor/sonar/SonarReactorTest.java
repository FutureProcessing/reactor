...
public class SonarReactorTest extends AbstractUnitTest {
    ...
    public void shouldSonarSingleMetricReturnStringReactorResponse() {
        // given
        given(sonarFacade.getSingleMetric(any(String.class), any(String.class))).willReturn(RESPONSE_STRING);
        SonarRequestData sonarRequestData = new SonarRequestData();
        
        // when        
        ReactorResponse response = sonarReactor.getSonarSingleMetric(new ReactorRequest<SonarRequestData>(null, null, sonarRequestData));
